  ...
  <div :class="isDark ? 'dark' : 'light'" class="flex min-h-screen" :style="{ backgroundColor: 'var(--bg-primary)' }">
    ...
    <aside class="fixed h-full z-50 flex flex-col transition-all duration-300"
      ...
      <button @click="collapsed = !collapsed"
        class="absolute -right-3 top-16 w-6 h-6 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 hover:scale-110 z-50"
      ...
      <nav class="flex-1 p-2 space-y-0.5 overflow-y-auto mt-2">
        <button v-for="menu in menus" :key="menu.path"
          ...
          class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 text-sm font-medium group relative"
          ...
          <div v-if="isActive(menu.path) && !collapsed"
            class="w-1.5 h-1.5 rounded-full bg-black opacity-50 shrink-0"></div>

          <!-- Hover background saat tidak aktif -->
          <div v-if="!isActive(menu.path)"
            ...
            @mouseover="$event.currentTarget.style.opacity='0.08'"
            @mouseleave="$event.currentTarget.style.opacity='0'">
          </div>

          <!-- Tooltip collapsed -->
          <div v-if="collapsed"
            class="absolute left-16 px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none z-50 shadow-lg"
      ...
      <div class="p-2 space-y-1" :style="{ borderTop: '1px solid var(--sidebar-border)' }">
        <button @click="toggleTheme"
          ...
          <MoonIcon v-if="isDark" class="w-5 h-5 shrink-0 group-hover:rotate-12 transition-transform duration-300" :style="{ color: 'var(--accent)' }" />
          <SunIcon v-else class="w-5 h-5 shrink-0 group-hover:rotate-45 transition-transform duration-300" :style="{ color: 'var(--accent)' }" />
          <transition name="slide-fade">
        ...
        <div class="flex items-center gap-2 px-3 py-2.5 rounded-xl"
          ...
          <transition name="slide-fade">
            <div v-if="!collapsed" class="flex-1 min-w-0">
              ...
              <p class="text-xs capitalize" :style="{ color: 'var(--text-muted)' }">Mekanik</p>
            </div>
          </transition>
          <button @click="logout"
            class="shrink-0 w-8 h-8 rounded-lg flex items-center justify-center hover:bg-red-500/20 hover:scale-110 transition-all duration-200">
...
import {
  HomeIcon, ClipboardDocumentListIcon, TruckIcon, CpuChipIcon, BanknotesIcon,
...
const menus = [
  { path: '/portal-mekanik', icon: HomeIcon, label: 'Dashboard' },
  { path: '/portal-mekanik/order', icon: ClipboardDocumentListIcon, label: 'Order Masuk' },
  { path: '/portal-mekanik/kendaraan', icon: TruckIcon, label: 'Kendaraan' },
  { path: '/portal-mekanik/penghasilan', icon: BanknotesIcon, label: 'Penghasilan' },
  { path: '/portal-mekanik/ai-diagnostic', icon: CpuChipIcon, label: 'AI Diagnostic' },
  { path: '/portal-mekanik/profil', icon: UserIcon, label: 'Profil Saya' },
]

const isActive = (path) => path === '/portal-mekanik' ? route.path === '/portal-mekanik' : route.path.startsWith(path)
