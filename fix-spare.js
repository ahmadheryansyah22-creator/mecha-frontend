    ...
    <div class="card">
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full">
          ...
          <tbody>
            <tr v-for="part in filteredParts" :key="part.id" class="border-b border-gray-800/50 hover:bg-gray-800/20" :class="part.stock <= part.min_stock ? 'bg-red-500/5' : ''">
              <td class="py-3">
              ...
              <td class="py-3"><span class="text-xs px-2 py-1 rounded-full" :class="statusClass(part.status)">{{ part.status }}</span></td>
              <td class="py-3">
      ...
      <div class="bg-[#1a1a1a] rounded-2xl p-8 border border-gray-800 w-full max-w-lg">
        ...
        <form @submit.prevent="storePart">
          <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
              <label class="block text-gray-400 text-sm mb-2">Nama Sparepart</label>
              <input v-model="form.name" type="text" class="input-field" required />
            </div>
            <div>
              <label class="block text-gray-400 text-sm mb-2">Kode</label>
            ...
            <div>
              <label class="block text-gray-400 text-sm mb-2">Merek</label>
              <input v-model="form.brand" type="text" class="input-field" />
            </div>
            <div>
              <label class="block text-gray-400 text-sm mb-2">Harga (Rp)</label>
              <input v-model="form.price" type="number" class="input-field" required />
            </div>
            <div>
              <label class="block text-gray-400 text-sm mb-2">Stok Awal</label>
              <input v-model="form.stock" type="number" class="input-field" required />
            </div>
            <div>
              <label class="block text-gray-400 text-sm mb-2">Min. Stok</label>
              <input v-model="form.min_stock" type="number" class="input-field" />
            </div>
            <div>
              <label class="block text-gray-400 text-sm mb-2">Satuan</label>
            ...
            <div class="col-span-2">
              <label class="block text-gray-400 text-sm mb-2">Bengkel</label>
...
fs.writeFileSync('src/views/SparePartView.vue', sparePartView, 'utf8');
